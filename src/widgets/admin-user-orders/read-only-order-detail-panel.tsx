import type { BackendAdminOrderDetail } from "@/shared/api/backend-types";
import {
...
export function adminPaymentStatusTone(status: string | undefined): StatusTone {
...
export function adminPaymentStatusLabel(status: string) {
...
export function ReadOnlyOrderDetailPanel({ order }: { order: BackendAdminOrderDetail }) {
    ...
    return (
        <div className="space-y-5 rounded-[1.75rem] border border-outline-variant/15 bg-surface-container-low p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <FieldValue
                    label="Da giao"
                    value={order.delivered_at ? formatDate(order.delivered_at) : "Chua giao xong"}
                />
            </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
                <div className="space-y-3 rounded-2xl bg-surface p-4">
                    ...
                    {order.status_history.length ? (
                        ...
                        <p className="text-sm text-on-surface-variant">Chua co lich su trang thai.</p>
                    )}
                </div>
                ...
                <div className="space-y-3 rounded-2xl bg-surface p-4">
                    ...
                    {order.payment_status_history.length ? (
                        order.payment_status_history.map((history) => (
                            <div key={history.id} className="rounded-2xl bg-surface-container-low p-4 text-sm">
                                <p className="font-medium text-on-surface">
                        ...
                        <p className="text-sm text-on-surface-variant">Chua co lich su thanh toan.</p>
                    )}
                </div>
            </div>
        </div>
