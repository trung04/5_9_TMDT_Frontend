...
import type { CustomerOrderDetailView } from "@/shared/api/storefront-adapters";
...
type CheckoutPaymentMethod = "COD" | "BANK_TRANSFER";

type CheckoutCartItemView = {
    id: string;
    productId: string;
    quantity: number;
    unitPrice: number;
...
function MockQrCode({ value }: { value: string }) {
    ...
    return (
        <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48 rounded-2xl bg-white p-3 shadow-sm">
...
function CopyButton({ onClick }: { onClick: () => void }) {
    return (
        <button
            className="inline-flex items-center gap-2 rounded-full border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700"
            onClick={onClick}
        >
...
export function CheckoutPage() {
    ...
    const [createdBankTransferOrder, setCreatedBankTransferOrder] = useState<CustomerOrderDetailView | null>(
        null,
    );
    ...
    const accountName =
        paymentInstructionValue(bankTransferPayload, "account_name") || "HERITAGE HARVEST";
    const accountNumber =
        paymentInstructionValue(bankTransferPayload, "account_number") || "0123456789";
    const transferContent =
        paymentInstructionValue(bankTransferPayload, "transfer_content") ||
    ...
    async function handlePlaceOrder() {
        ...
        const insufficientItem = cartItems.find(
            (item) => item.product.stockQuantity !== undefined && item.quantity > item.product.stockQuantity,
        );

        if (insufficientItem) {
            setSubmitError(
                `Sản phẩm ${insufficientItem.product.name} chỉ còn ${insufficientItem.product.stockQuantity}, không đủ cho số lượng ${insufficientItem.quantity}.`,
            );
            return;
        }
                                            ...
                                            <div className="mt-4 flex items-center justify-between gap-4">
                                                <div className="flex items-center rounded-full bg-surface-container px-2 py-1">
                                                    <button
                                                        ...
                                                        onClick={() => void setQuantity(item.productId, Math.max(1, item.quantity - 1))}
                    ...
                    <div className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[1.5rem] border border-emerald-100 bg-white p-5 shadow-2xl">
                            ...
                            <div className="space-y-3 rounded-[1.25rem] border border-emerald-200 bg-emerald-100/90 p-4">
                                ...
                                <div className="rounded-2xl bg-white/95 px-4 py-4 shadow-sm">
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <CopyButton
                                            onClick={() => void handleCopy(String(createdBankTransferOrder.totalAmount))}
                                        />
                                    </div>
                                </div>
                                <div className="rounded-2xl bg-white/95 px-4 py-4 shadow-sm">
                        ...
                        <div className="mt-5 flex flex-wrap justify-center gap-3">
                            ...
                            <button
                                ...
                                onClick={() => void handleConfirmTransferSubmitted()}
                            >
