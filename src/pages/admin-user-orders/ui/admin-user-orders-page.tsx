...
import type { BackendAdminOrderSummary } from "@/shared/api/backend-types";
...
} from "@/widgets/admin-user-orders/read-only-order-detail-panel";
...
export function AdminUserOrdersPage() {
    ...
    const customerOrdersPaginationByCustomerId = useAdminUserStore(
        (state) => state.customerOrdersPaginationByCustomerId,
    );
    const customerOrdersLoadingByCustomerId = useAdminUserStore(
        (state) => state.customerOrdersLoadingByCustomerId,
    );
    const customerOrdersErrorByCustomerId = useAdminUserStore(
        (state) => state.customerOrdersErrorByCustomerId,
    );
        ...
        return (
            <div className="space-y-6">
                <AdminPageHeader
                    ...
                    description="Khong the mo trang lich su don hang cho customer nay."
                    actions={
                        <ButtonLink to={routes.adminUsers} variant="secondary">
                            Quay lai users
                        </ButtonLink>
                    }
                />
        ...
        <div className="space-y-8">
            ...
            <SurfaceCard className="space-y-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {customer ? (
                            <Badge tone={customer.is_active && !customer.is_deleted ? "success" : "warning"}>
            ...
            {!ordersLoading && orders.length > 0 ? (
                <div className="space-y-4">
                    {orders.map((order: BackendAdminOrderSummary) => (
                        <SurfaceCard key={order.id} className="space-y-4">
                            <div className="flex flex-wrap items-start justify-between gap-4">
                                ...
                                <div className="flex min-w-[12rem] flex-col items-end gap-3">
                                    ...
                                    <ButtonLink
                                        to={routes.adminUserOrderDetail(String(customerId), String(order.id))}
                                        variant="outline"
                                        size="sm"
                                    >
                                        Xem chi tiet
                    ...
                    {pagination && pagination.lastPage > 1 ? (
                        <SurfaceCard className="flex flex-wrap items-center justify-between gap-3 text-sm">
                            <p className="text-on-surface-variant">
                            ...
                            <div className="flex gap-3">
                                <Button
                                    variant="secondary"
                                    size="sm"
                                    disabled={ordersLoading || pagination.currentPage <= 1}
                                ...
                                <Button
                                    variant="secondary"
                                    size="sm"
                                    disabled={ordersLoading || pagination.currentPage >= pagination.lastPage}
                                    onClick={() =>
