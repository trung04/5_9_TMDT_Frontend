...
export function AccountOrdersPage() {
    ...
    async function handleReorder(goToCheckout: boolean) {
        ...
        for (const item of activeOrder.items) {
            ...
            if (!result.success) {
                setReorderMessage(result.error ?? "Khong the them lai toan bo san pham vao gio.");
                return;
            }
        }

        setReorderMessage(
            `Da them lai ${activeOrder.items.length} mat hang tu don ${activeOrder.orderNo} vao gio.`,
        );
            ...
            <div className="mx-auto max-w-7xl px-6 pb-16 pt-24">
                <div className="rounded-xl bg-surface-container-low p-10 text-center">
                    ...
                    <p className="mt-3 text-on-surface-variant">
                        Sau khi checkout thanh cong, don se xuat hien tai day va duoc cap nhat tu dong.
                    </p>
                    <button
                        className="mt-6 rounded-full bg-primary px-6 py-3 font-semibold text-on-primary"
            ...
            <aside className="flex w-full flex-col gap-6 md:w-80">
                <div className="rounded-xl bg-surface-container-lowest p-8">
                    <div className="flex flex-col items-center space-y-4 text-center">
                        ...
                        <div>
                            <h2 className="font-headline text-xl font-bold text-on-surface">
                                {profile.name || "Khach hang"}
                            </h2>
                            <p className="text-sm text-on-surface-variant">
                                {profile.memberSince
                                    ? `Thanh vien tu ${formatDate(profile.memberSince)}`
                                    : "Ho so tai khoan dang duoc cap nhat"}
                            </p>
                        </div>
                    </div>
                    <div className="mt-8 space-y-2">
                        <div className="flex items-center gap-3 rounded-lg bg-white px-4 py-3 font-semibold text-green-800 shadow-sm">
                            ...
                            <span className="text-sm">Don hang</span>
                        </div>
                        <button
                        ...
                        >
                            <Icon name="gavel" />
                            <span className="text-sm">Khieu nai va ho tro</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="relative overflow-hidden rounded-xl bg-tertiary-container p-6 text-on-tertiary-container">
                    <div className="relative z-10">
                        <h3 className="mb-2 text-lg font-bold">Diem thuong hoi vien</h3>
                        <p className="mb-4 text-xs opacity-90">
                            Ban dang co {rewards.points} diem thuong tren tai khoan.
                        </p>
                        <button
                ...
                <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <p className="mt-1 text-on-surface-variant">
                            Theo doi don hang va nhan cap nhat trang thai moi nhat.
                        </p>
                    </div>
                    <div className="flex items-center gap-3 self-start sm:self-auto">
                        ...
                        >
                            ...
                            {showFilterBar ? "An bo loc" : "Hien bo loc"}
                        </button>
                    </div>
                </header>
                        ...
                        <div className="border-b border-outline-variant/15 bg-gradient-to-r from-surface-container-lowest to-surface-container-low p-6">
                            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                                <input
                                    ...
                                    placeholder="Tim theo ma don hoac trang thai..."
                                    value={searchQuery}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse text-left">
                            <thead className="border-b border-outline-variant/15 bg-surface-container-low">
                                <tr>
                                    {["Ma don", "Ngay", "Tong tien", "Trang thai", "Thao tac"].map(
                                        (title) => (
                                            <th
                                                key={title}
                            ...
                            {activeOrder ? (
                                <>
                                    ...
                                    <div className="space-y-4 rounded-xl bg-surface-container-lowest p-5">
                                        <p className="text-xs uppercase tracking-widest text-on-surface-variant">
                                            Trang thai va thanh toan
                                        </p>
                                        <div className="grid gap-4 md:grid-cols-2">
                                            <div>
                                                <p className="text-sm text-on-surface-variant">Trang thai don</p>
                                            ...
                                            <div>
                                                <p className="text-sm text-on-surface-variant">Trang thai thanh toan</p>
                                                <p className="mt-1 font-semibold">
                                                    {activeOrder.payment
                                                        ? customerPaymentStatusLabels[
                                        ...
                                        <div>
                                            <p className="text-sm text-on-surface-variant">Dia chi nhan</p>
                                            <p className="mt-1 font-semibold">{activeOrder.shippingAddress}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-on-surface-variant">Ghi chu</p>
                                            <p className="mt-1 font-semibold">
                                                {activeOrder.note || "Khong co ghi chu."}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        {activeOrder.statusHistory.length > 0 ? (
                                            ...
                                            <div className="rounded-xl bg-surface-container-lowest p-5 text-sm text-on-surface-variant">
                                                Chua co lich su trang thai cho don nay.
                                            </div>
                                        )}
                                    </div>
                                </>
                        ...
                        <div className="space-y-5 rounded-xl bg-surface-container-lowest p-6">
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p className="mt-2 text-sm text-on-surface-variant">
                                        Muc khieu nai hien chua ho tro gui truc tiep. Chung toi se cap nhat trong thoi
                                        gian som nhat.
                                    </p>
                                </div>
                            ...
                            <div className="rounded-xl bg-surface-container-low p-5 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-on-surface-variant">Ma don</span>
                                    <span className="font-semibold">{activeOrderSummary.orderNo}</span>
                                </div>
                                <div className="mt-2 flex justify-between">
                                    <span className="text-on-surface-variant">Tong don</span>
                                    <span className="font-semibold">
                                        {formatCurrency(activeOrderSummary.totalAmount)}
                                    </span>
                                </div>
                                <div className="mt-2 flex justify-between">
                                    <span className="text-on-surface-variant">Thanh toan</span>
                                    <span className="font-semibold">
                            ...
                            <div className="rounded-xl border border-dashed border-outline-variant/30 p-5 text-sm leading-7 text-on-surface-variant">
                                Khi co tinh nang khieu nai, khu vuc nay se duoc mo de gui yeu cau truc tiep cho don
                                hang dang chon.
                            </div>
                        </div>
                    </div>
