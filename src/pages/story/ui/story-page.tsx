...
export function StoryPage() {
    ...
    const canInteract = Boolean(
        accessToken && authSource === "backend" && session?.user.role === "customer",
    );
    ...
    async function handleToggleLike(post: BackendPost) {
        if (!canInteract) {
            pushToast({
                tone: "warning",
    ...
    async function handleSubmitComment(post: BackendPost) {
    ...
    return (
        <div className="mx-auto max-w-5xl px-6 pb-12 pt-24">
            ...
            {status === "error" ? (
                <SurfaceCard className="text-sm text-error">
            ...
            <div className="space-y-6">
                {posts.map((post) => {
                    ...
                    return (
                        <SurfaceCard key={post.id} className="overflow-hidden p-0">
                            {post.cover_image_url ? (
                                ...
                                <div className="flex h-44 w-full items-center justify-center bg-surface-container-low text-primary">
                            ...
                            <div className="space-y-6 p-6">
                                ...
                                <div className="space-y-3 text-sm leading-7 text-on-surface">
                                    {paragraphLines(post.body).map((line) => (
                                        <p key={line}>{line}</p>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-wrap items-center gap-3 border-y border-outline-variant/15 py-4">
                                    <Button
                                        ...
                                        size="sm"
                                        iconLeft={<Icon name="favorite" fill={isLiked} />}
                                        disabled={isSaving}
                                ...
                                <section className="space-y-4">
                                    ...
                                    {canInteract ? (
                                        ...
                                        <div className="flex flex-col gap-3 rounded-2xl bg-surface-container-low p-4 text-sm text-on-surface-variant sm:flex-row sm:items-center sm:justify-between">
