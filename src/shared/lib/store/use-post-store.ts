...
import type {
    BackendMyPostLikesResponse,
    BackendPost,
    BackendPostComment,
    BackendPostCommentResponse,
    BackendPostLikesResponse,
    BackendPostResponse,
...
interface PostState {
    posts: BackendPost[];
    likedPostIds: number[];
    adminPosts: BackendPost[];
    status: LoadStatus;
    adminStatus: LoadStatus;
    error: string | null;
    adminError: string | null;
    isSaving: boolean;
    loadPosts: (force?: boolean) => Promise<AsyncResult>;
    loadMyLikes: () => Promise<AsyncResult<number[]>>;
    toggleLike: (postId: number) => Promise<AsyncResult<BackendPostLikesResponse["data"]>>;
    addComment: (postId: number, content: string) => Promise<AsyncResult<BackendPostComment>>;
    loadAdminPosts: () => Promise<AsyncResult>;
    createPost: (payload: PostPayload) => Promise<AsyncResult<BackendPost>>;
    updatePost: (postId: number, payload: PostPayload) => Promise<AsyncResult<BackendPost>>;
    deletePost: (postId: number) => Promise<AsyncResult>;
    updateCommentVisibility: (
        commentId: number,
        status: "VISIBLE" | "HIDDEN",
    ) => Promise<AsyncResult<BackendPostComment>>;
    reset: () => void;
}

const initialState = {
    posts: [] as BackendPost[],
    likedPostIds: [] as number[],
    adminPosts: [] as BackendPost[],
    status: "idle" as LoadStatus,
    adminStatus: "idle" as LoadStatus,
...
function replacePost(posts: BackendPost[], nextPost: BackendPost) {
    return posts.map((post) => (post.id === nextPost.id ? nextPost : post));
}

function removeCommentFromPublicPosts(posts: BackendPost[], comment: BackendPostComment) {
    return posts.map((post) => {
        ...
        return {
            ...
            comments_count:
                hadComment && post.comments_count > 0 ? post.comments_count - 1 : post.comments_count,
        };
    });
}
...
export const usePostStore = create<PostState>()((set, get) => ({
    ...
    loadPosts: async (force = false) => {
        ...
        try {
            const response = await apiRequest<BackendPostsResponse>("/posts");
            set({
                posts: response.data,
                status: "ready",
                error: null,
    ...
    loadMyLikes: async () => {
        ...
        try {
            const response = await apiRequest<BackendMyPostLikesResponse>("/posts/my-likes", {
                token: accessToken,
            });
    ...
    toggleLike: async (postId) => {
        ...
        try {
            const response = await apiRequest<BackendPostLikesResponse>(`/posts/${postId}/likes`, {
    ...
    addComment: async (postId, content) => {
        ...
        try {
            const response = await apiRequest<BackendPostCommentResponse>(`/posts/${postId}/comments`, {
                method: "POST",
                token: accessToken,
                body: { content },
            });
    ...
    loadAdminPosts: async () => {
        ...
        try {
            const response = await apiRequest<BackendPostsResponse>("/admin/posts", {
                token: accessToken,
            });
            set({
    ...
    createPost: async (payload) => {
        const accessToken = token();
        if (!accessToken) return { success: false, error: "Bạn cần đăng nhập admin để tạo bài viết." };

        set({ isSaving: true, adminError: null });

        try {
            const response = await apiRequest<BackendPostResponse>("/admin/posts", {
                method: "POST",
                token: accessToken,
                body: payload,
            });
    ...
    updatePost: async (postId, payload) => {
        const accessToken = token();
        if (!accessToken) return { success: false, error: "Bạn cần đăng nhập admin để cập nhật bài viết." };

        set({ isSaving: true, adminError: null });

        try {
            const response = await apiRequest<BackendPostResponse>(`/admin/posts/${postId}`, {
                method: "PUT",
                token: accessToken,
                body: payload,
            });
    ...
    deletePost: async (postId) => {
        const accessToken = token();
        if (!accessToken) return { success: false, error: "Bạn cần đăng nhập admin để xóa bài viết." };
    ...
    updateCommentVisibility: async (commentId, status) => {
        const accessToken = token();
        if (!accessToken) return { success: false, error: "Bạn cần đăng nhập admin để quản lý bình luận." };

        set({ isSaving: true, adminError: null });

        try {
            const response = await apiRequest<BackendPostCommentResponse>(
                `/admin/posts/comments/${commentId}/visibility`,
                {
                    method: "PATCH",
                    token: accessToken,
                    body: { status },
